<template>
    <div class="card p-0">
        ...
        <div class="text-center text-sm text-gray-100 my-1" v-if="referrers.length == 0">No Data</div>
...
export default {
    ...
    methods: {
        async fetch() {
            await fetch(`/manage/statamic-analytics/api/top-referrers?period=${this.period}`)
                .then(res => res.json())
